...
本功能目前是经验阈值试验，没有细类独立测试集或准确率声明。检测先运行 YOLO；再用颜色/形状自动生成补充候选，只有通过图示及必要 OCR 复核的候选才进入输出，结果标明来源。已对 123 张原始道路图做探索性检查；补充候选使具体解读从 11 个增至 13 个，但大量候选仍被拒绝。实现与局限见[阶段记录](docs/small_sign_prototype_20260926.md)和[检测方法迭代报告](docs/small_sign_detection_iteration_20260926.md)。
