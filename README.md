...
这个原有的 `lookup_sign` 工具仍只是**四类 YAML 的关键词/别名查询**，不是向量检索。验收：`scripts\verify_knowledge.py`（离线；`--live-llm` 追加真实 API 调用检查）。下文另有公开项目文档的两步式 RAG 试验，两者不要混称。

## 清晰小标志含义匹配（无人工细类标注的试验）

页面中的独立区域「清晰小标志含义匹配」从道路原图检测 0/1/3 类，对足够清晰且较大的独立牌面尝试图示匹配，并展示证据框、来源和拒答原因。参考图像首次从[北京市交管局图解](https://jtgl.beijing.gov.cn/jgj/lszt/659722/10860935/)获取，仅缓存在本机 `.runtime`；图像匹配与 OCR 不调用 DeepSeek，也不会把用户图片上传。现有图示目录有 163 个实例，**只有其中 22 个名称附有初步释义并参与输出**；其他标志、尺寸不足、图案相近、数值读不清的情况拒答。限速读数来自当前图片 OCR，不复用图解的示例数字。辅助牌尚未解析。

本功能目前是经验阈值试验，没有细类独立测试集或准确率声明。检测先运行 YOLO；再用颜色/形状自动生成补充候选，只有通过图示及必要 OCR 复核的候选才进入输出，结果标明来源。针对清晰人行横道牌，另核验蓝底白三角与黑色行人/横道笔画；对易混淆的“环岛行驶”暂不凭通用图案相似度给出含义。在 123 张原始道路图的探索批次上，具体解读由 13 个增至 19 个，新增 6 个均已逐图目视核对；这批样例参与规则调整，**不是独立精度成绩**。实现与局限见[阶段记录](docs/small_sign_prototype_20260926.md)、[检测方法迭代报告](docs/small_sign_detection_iteration_20260926.md)及[人行横道与误判拒答报告](docs/small_sign_pedestrian_iteration_20260926.md)。

## 公开项目资料问答（RAG 第一阶段）

页面底部新增独立的知识问答区。它从仓库公开的 `README.md`、`docs/direction_upgrade_v3.md` 和四类 `knowledge/signs.yaml` 中拆出 45 段，按中文字符片段、词和别名做 BM25 检索。离线模式只展示原文摘录及文件行号；DeepSeek 模式先检索再生成回答，并要求引用命中段落的编号。缺少引用或引用了不存在的编号时返回“资料不足”。问题及命中的**公开文字段落**会发送给 DeepSeek，图片、本机数据集和密钥不会作为检索资料发送。

```powershell
.\.venv\Scripts\python.exe scripts\evaluate_rag.py          # 离线开发题集，不调用 API
.\.venv\Scripts\python.exe scripts\verify_rag.py --live-llm # 三次真实 API 问答
```

首阶段开发题集 20 道有答案题的预期主题 Top-3 命中 20/20，8 道无答案题中正确拒绝 7/8；“车规认证编号”被不相关的“编号”文字误召回。真实 API 联调的两道有依据题均给出可追溯引用，一道无依据题拒答。这些题由开发者编写并用于调试，**不是独立测试成绩**；引用编号检查也不能证明回答的每句话都正确。当前没有 embedding、向量数据库、语义重排、图片知识融合或独立人审。详情见 [阶段报告](docs/rag_stage1_20260926.md)。
...
缓存键包含规范化图片内容、尺寸和阈值。同会话重复问答复用检测；换图或调阈值清空历史与折叠上下文。历史最多保留最近 2 轮，更早轮次折叠进结构化上下文。离线视觉问答模式按明确关键词工作；文档问答的离线模式只返回证据摘录。未实现向量检索、自进化、LangChain 或多模型路由。
